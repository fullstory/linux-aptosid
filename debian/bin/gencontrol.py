    ...
    def process_real_tree(self, in_entry, vars):
        ...
        for i in (('Depends', 'Provides')):
            ...
            if i == 'Depends':
                ...
                value.append("linux-patch-sidux-%s (= %s)" % (v.linux_version, v.complete))
                value.append(' | '.join(["linux-source-sidux-%s (= %s)" % (v.linux_version, v.complete) for v in self.versions]))
            elif i == 'Provides':
                value.extend(["linux-tree-sidux-%s" % v.complete.replace('~', '-') for v in self.versions])
            entry[i] = value
        return entry
